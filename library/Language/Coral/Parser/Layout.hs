module Language.Coral.Parser.Layout
  ( fixLayout
  , getIndentLine
  )
where
...
import           Language.Coral.Parser.ErrM
import           Language.Coral.Parser.LayoutGrammar
import           Language.Coral.Parser.LexGrammar
...
fixLayout tp = res Nothing [if tl then Implicit 1 else Explicit]
 where
  tl = tp && topLayout
  res
  ...
  res pt st rest@(t0 : ts)
    ...
    = let st' = drop 1 (dropWhile isImplicit st)
      in  if null st' then layoutErrorAt t0 else moveAlong st' [t0] ts
  ...
  res pt st@(Implicit n : ns) rest@(t0 : ts) |
    ...
    if isNothing pt
      ...
      then moveAlong st [t0] ts
        ...
        let b : t0' : ts' = addToken (afterPrev pt) layoutSep rest
...
getIndentLine ts = case runState (go ts) (IndentLine 0 []) of
  (Just (i0, r), s) -> (Just (i0, reverse $ toks s), r)
  (Nothing     , s) -> (Nothing, toks s)
 where
  addToLine :: Token -> State IndentLine ()
  addToLine t = modify $ \IndentLine { count = c, toks } ->
    let c' | isOpenParen t  = succ c
  ...
  go rest@(r : rs') = do
    ...
    if null tks || not (isIndentTok tk && c == 0)
      ...
      else pure $ case pIndentLine (reverse tks) of
        Ok  _ -> Just (tk, rest)
        Bad e -> Nothing
