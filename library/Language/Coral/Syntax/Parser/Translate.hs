...
-}
module Language.Coral.Syntax.Parser.Translate (translate) where
...
translProgram (AG.Prog _ defs) = AST.Module (map translDef priv)
                                            (map translDef pub)
  where
...
translDef (AG.Definition1 _ (AG.AsyncDef _ funcDef))        = AST.AsyncFun $ translFunc funcDef
...
translDef (AG.DefinitionAsyncDef _ (AG.AsyncDef _ funcDef)) = AST.AsyncFun $ translFunc funcDef
...
translFunc (AG.FuncDef _ name params retType suite) = AST.Fun (translName name)
...
translTypeDef (AG.RecDef _ typeName fields) = AST.Record (translTypeName typeName)
                                                         (map translField fields)
...
translField (AG.Field _ name type') = AST.Field (translName name)
                                                (translType type')


translTypeName ∷ forall a. AG.TypeName a → AST.Type
translTypeName (AG.Name _ con)       = AST.Type [translCon con]
translTypeName (AG.GenName _ con ts) = AST.GenType [translCon con]
                                                   (map translType ts)
...
translDataDef (AG.DataDef _ typeName dfs) = AST.DataDef (translTypeName typeName)
  ...
  where
    ...
    translDataField (AG.DataFieldProp _ (AG.Prop _ name type'))  = AST.Prop (translName name)
                                                                            (translType type')
...
translParam (AG.Parameter1 _ name argtype)     = AST.Param (translName name)
                                                           (translArgType argtype)
                                                           Nothing
translParam (AG.Parameter2 _ name argtype def) = AST.Param (translName name)
                                                           (translArgType argtype)
                                                           (Just (translTest def))
translParam (AG.Parameter3 _ name argtype)     = AST.VarParam (translName name)
                                                              (translArgType argtype)
...
translType (AG.GenericType _  type' args) = AST.GenType [translCon type']
                                                        (map translType args)
translType (AG.ConcreteType _ type')      = AST.Type [translCon type']
translType (AG.ParamType _ type')         = AST.FreeType $ translGen type'
translType (AG.FunType _ args ret)        = AST.FunType (map translType args)
                                                        (translType ret)
...
translStmt (AG.MutAssign _ test expr)       = AST.MutAssign (translTest test) (translExpr expr)
translStmt (AG.DefVal _ name varType expr)  = AST.Declaration (translName name)
                                                              (translVarType varType)
                                                              (Just $ translExpr expr)
translStmt (AG.Def _ name varType)          = AST.Declaration (translName name)
                                                              (translVarType varType)
                                                              Nothing
translStmt (AG.MutDefVal _ name varType e)  = AST.MutDeclaration (translName name)
                                                                 (translVarType varType)
                                                                 (Just $ translExpr e)
translStmt (AG.MutDef _ name varType)       = AST.MutDeclaration (translName name)
                                                                 (translVarType varType)
                                                                 Nothing
...
translStmt (AG.WhileElse _ be s1 s2)        = AST.While (translBoolExpr be)
                                                        (translSuite s1)
                                                        (Just $ translSuite s2)
translStmt (AG.While    _ be s1)            = AST.While (translBoolExpr be) (translSuite s1) Nothing
translStmt (AG.ForElse _ ts1 ts2 s1 s2)     = AST.For (map translTest ts1) (map translTest ts2)
                                                      (translSuite s1) (Just $ translSuite s2)
translStmt (AG.For     _ ts1 ts2 s)         = AST.For (map translTest ts1) (map translTest ts2)
                                                      (translSuite s) Nothing
...
translStmt (AG.IfElif _ be s els)           = AST.Conditional guards Nothing
  where guards = ((translBoolExpr be, translSuite s):)
               $ els <&> \(AG.Elif _ e s') -> (translBoolExpr e, translSuite s')
translStmt (AG.If     _ be s)               = AST.Conditional guard Nothing
  where guard = [(translBoolExpr be, translSuite s)]
translStmt (AG.TryEFin  _    s excps se sf) = AST.Try { AST.body=translSuite s
                                                      , AST.excepts=translExcept <$> excps
                                                      , AST.handlers=[]
                                                      , AST.else'=Just $ translSuite se
                                                      , AST.finally=Just $ translSuite sf}
translStmt (AG.TryWEFin _ hs s excps se sf) = AST.Try { AST.body=translSuite s
                                                      , AST.excepts=translExcept <$> excps
                                                      , AST.handlers=translTest <$> hs
                                                      , AST.else'=Just $ translSuite se
                                                      , AST.finally=Just $ translSuite sf}
translStmt (AG.TryElse  _    s excps se)    = AST.Try { AST.body=translSuite s
                                                      , AST.excepts=translExcept <$> excps
                                                      , AST.handlers=[]
                                                      , AST.else'=Just $ translSuite se
                                                      , AST.finally = Nothing}
translStmt (AG.TryWElse _ hs s excps se)    = AST.Try { AST.body=translSuite s
                                                      , AST.excepts=translExcept <$> excps
                                                      , AST.handlers=translTest <$> hs
                                                      , AST.else'=Just $ translSuite se
                                                      , AST.finally=Nothing}
translStmt (AG.Try      _    s excps)       = AST.Try { AST.body=translSuite s
                                                      , AST.excepts=translExcept <$> excps
                                                      , AST.handlers=[]
                                                      , AST.else'=Nothing
                                                      , AST.finally=Nothing}
translStmt (AG.TryWith  _ hs s excps)       = AST.Try { AST.body=translSuite s
                                                      , AST.excepts=translExcept <$> excps
                                                      , AST.handlers=translTest <$> hs
                                                      , AST.else'=Nothing
                                                      , AST.finally=Nothing}
...
translExcept (AG.EExcp _    s) = AST.Handler { AST.clause = AST.CatchAll
                                             , AST.hanBody = translSuite s
                                             }
translExcept (AG.Excp  _ ts s) = AST.Handler { AST.clause = AST.Catch $ map translClause ts
                                             , AST.hanBody = translSuite s}
  where
...
translTest (AG.Test1 _ name trails) = foldl' translTrailer
  ...
  where
    ...
    translSubs (AG.Subscript1 _ t1 t2 t3) = AST.Slice (translSliceTerm t1)
                                                      (translSliceTerm t2)
                                                      (translSliceTerm t3)
    translSubs (AG.Subscript2 _    t2 t3) = AST.Slice Nothing
                                                      (translSliceTerm t2)
                                                      (translSliceTerm t3)
    translSubs (AG.Subscript3 _ t1    t3) = AST.Slice (translSliceTerm t1)
                                                      Nothing
                                                      (translSliceTerm t3)
    translSubs (AG.Subscript4 _ t1 t2)    = AST.Slice (translSliceTerm t1)
                                                      (translSliceTerm t2)
                                                      Nothing
    translSubs (AG.Subscript5 _ t1)       = AST.Slice (translSliceTerm t1)
                                                      Nothing
                                                      Nothing
    translSubs (AG.Subscript6 _    t2)    = AST.Slice Nothing
                                                      (translSliceTerm t2)
                                                      Nothing
    translSubs (AG.Subscript7 _       t3) = AST.Slice Nothing
                                                      Nothing
                                                      (translSliceTerm t3)
    ...
    translArg (AG.Arg1 _ name' test) = AST.KeywordArg (translName name')
                                                      (translTest test)
